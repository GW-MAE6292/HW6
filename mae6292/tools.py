...
def compute_disparity(img_left, img_right, W_patch, d_min, d_max, outlier_ratio=1.5):
    ...
    for i_row in range(W_patch, N_rows-W_patch):
        for i_col in range(W_patch+d_max, N_cols-W_patch):
            ...
            if SSD[SSD <= SSD_min*outlier_ratio].shape[0] < 3 and i_match != 0 and i_match != SSD.shape[0]-1:
...
def disparity2PC(disparity, K, baseline):
